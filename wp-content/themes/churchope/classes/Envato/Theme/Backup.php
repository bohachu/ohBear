...
class Envato_Theme_Backup
{
	...
	public function check_archive()
	{
		...
		if (!file_exists($this->archive_filepath()))
			$this->error($this->archive_method(), __('The backup file was not created', 'churchope'));
...
//					continue;
		...
		if (!empty($this->unreadable_files))
			$this->warning($this->archive_method(), __('The following files are unreadable and could not be backed up: ', 'churchope') . implode(', ', $this->unreadable_files));
