...
abstract class Admin_Theme_Menu_Item
{
	...
	function __construct($parent_slug = '')
	{
		if (strlen($parent_slug))
		{
			...
			add_submenu_page($this->getParentSlug(), $this->getPageTitle(), $this->getMenuTitle(), $this->getCapability(), $this->getMenuSlug(), array($this, 'renderPage'));
		}
	}
	...
	public function renderPage()
	{
		...
		if ($this->getMenuSlug() != SHORTNAME . '_dummy')
		{
			...
			<form method="post" id="th_reset"  action="">' .
					$reset_nonce
					. '<input type="hidden" name="reset_options" id="reset_options" /><input name="reset" type="submit" value="' . __('Reset', 'churchope') . '" class="th_reset" />
			</form>';
		}
