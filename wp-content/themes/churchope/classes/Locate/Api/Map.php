...
class Locate_Api_Map extends Locate_Api
{
	...
	{
		$map = serialize($this);
		return update_post_meta($postid, Locate_Api_Map::getMetaKey(), $map);
	}
...
//			$html .= apply_filters('location_map_directions_html', null, $this, $options);
	 ...
	 * @param mixed $options
	 */
	static function _directions_html($html, $map, $options)
	{
		...
		$html = "
			<div id='{$options->mapName}_directions' class='location-map-directions'>
				<form action=''>
			<p><span id='{$options->mapName}_car_button' class='location-map-car-button location-map-travelmode selected' title='" . __('By car', 'churchope') . "' ></span><span id='{$options->mapName}_bike_button' class='location-map-bike-button location-map-travelmode' title='" . __('Bicycling', 'churchope') . "' ></span><span id='{$options->mapName}_walk_button' class='location-map-walk-button location-map-travelmode' title='" . __('Walking', 'churchope') . "' ></span></p>
