	...
	function widget($args, $instance)
	{
		...
		if (is_array($upcoming_events) && count($upcoming_events))
		{
			...
			{		
				...
				$mt = date_i18n("M",  strtotime('1 '.$event->month));
				...
				<p><a href="{$url}" class="entry-title">
						...
						if ($event->is_repeat)
						{
							...
							if ($event->time_left > 0)
							{
								...
								for ($i = 0; $i < $count - 1; $i++)
								{
									...
									if (!in_array($one_of_event_timestamp, $skipped_dates))
									{
										if (!(isset($event->end_date) && $event->end_date != null && $one_of_event_timestamp > $end_event_datetimestamp))
										{
