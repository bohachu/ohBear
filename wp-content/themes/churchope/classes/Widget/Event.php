...
class Widget_Event extends Widget_Default
{
	 ...
	 * start date meta key
	 */
	const EVENT_START_DATE_META_KEY = '_event_date';
	...
	/**
	 * skipped days meta key
	 */
	const EVENT_SKIPPED_DATES_META_KEY = '_event_skipped_dates';
/////////////////////////////
		...
		<div>
			...
			<?php if (true || $events): ?>
				...
				<select name ="<?php echo $this->get_field_name(self::EVENT_FIELD); ?>" id="<?php echo $this->get_field_id(self::EVENT_FIELD); ?>" style="width:100%;">
					<?php
					$selected = "";
					if (isset($instance[self::EVENT_FIELD]) && $instance[self::EVENT_FIELD] == self::NEXT)
					{
						$selected = "selected='selected'";
					}
					...
					foreach ((array) $events as $event):
						if (isset($event->post_id))
						{
							...
							if ($event_post)
							{
								if (isset($instance[self::EVENT_FIELD]) && $instance[self::EVENT_FIELD] == $event->post_id)
								{
									$selected = "selected='selected'";
								}
								else
		...
		if ($wpdb)
		{
			...
			$query = <<<SQL
				SELECT
					...
					meta_start_date.meta_value as date,
					...
					(UNIX_TIMESTAMP( STR_TO_DATE( CONCAT(meta_start_date.meta_value, ' ', IF(ISNULL(meta_time.meta_value),'12:00 AM',meta_time.meta_value)),CONCAT('%m/%d/%Y' , ' ', IF(meta_time.meta_value REGEXP '[^m]$', '%H:%i', '%h:%i %p' )) ) ) - UNIX_TIMESTAMP() - {$this->getTimezoneShift()} ) as time_left
					...
					LEFT JOIN {$wpdb->postmeta} meta_start_date ON (
																postmeta.post_id = meta_start_date.post_id
																AND
																meta_start_date.meta_key = '{$this->getDateMetaKeyName()}'
															)
		...
		if ($wpdb)
		{
			...
			$query = <<<SQL
				SELECT
					...
					meta_skipped_dates.meta_value as skipped_dates,
					IF(ISNULL(meta_repeat.meta_value),0,IF(meta_repeat.meta_value = 'on',1,0)) as is_repeat,
					(UNIX_TIMESTAMP( STR_TO_DATE( CONCAT(meta_start_date.meta_value, ' ', IF(ISNULL(meta_time.meta_value),'12:00 AM',meta_time.meta_value)),CONCAT('%m/%d/%Y' , ' ', IF(meta_time.meta_value REGEXP '[^m]$', '%H:%i', '%h:%i %p' )) ) ) - UNIX_TIMESTAMP() - {$this->getTimezoneShift()} ) as time_left
					...
					LEFT JOIN {$wpdb->postmeta} meta_skipped_dates ON (
																postmeta.post_id = meta_skipped_dates.post_id
																AND
																meta_skipped_dates.meta_key = '{$this->getSkippedDatesMetaKeyName()}'
															)
SQL;
			...
			$query .= <<<SQL
					WHERE
						...
						AND (
							...
							OR
							(UNIX_TIMESTAMP( STR_TO_DATE( CONCAT(meta_start_date.meta_value, ' ', IF(ISNULL(meta_time.meta_value),'12:00 AM', meta_time.meta_value)),CONCAT('%m/%d/%Y' , ' ', IF(meta_time.meta_value REGEXP '[^m]$', '%H:%i', '%h:%i %p' )) ) ) - UNIX_TIMESTAMP() - {$this->getTimezoneShift()}) > 0
						)
					ORDER BY post_id
SQL;
	...
	public function getMonthEvents($month = null, $year = null)
	{
		...
		foreach ((array) $repeating_events as $event)
		{
			...
			$skipped_dates = $this->getSkippedDates($event);
			$one_of_event_date = 0;
...
//							$one_of_event_date = $this->getNextMonthEventDate($start_event_datetimestamp);
						...
						if ($one_of_event_date >= $month_start && $one_of_event_date <= $month_end && !in_array($one_of_event_date, $skipped_dates))
						{
		...
		if ($wpdb)
		{
			...
			$query = <<<SQL
				SELECT
					postmeta.post_id as post_id,
					meta_start_date.meta_value as date,
					DAY(STR_TO_DATE(meta_start_date.meta_value, '%m/%d/%Y')) as day,
					meta_time.meta_value as time,
					meta_end_date.meta_value as end_date,
	...
	private function getNotRepeatingMonthEvents($month = null, $year = null)
	{
		...
		if ($wpdb)
		{
			...
			SELECT
					...
					meta_interval.meta_value as event_interval,
					IF(ISNULL(meta_repeat.meta_value),0,IF(meta_repeat.meta_value = 'on',1,0)) as is_repeat,
					(UNIX_TIMESTAMP( STR_TO_DATE( CONCAT(meta_start_date.meta_value, ' ', meta_time.meta_value),CONCAT('%m/%d/%Y' , ' ', IF(meta_time.meta_value REGEXP '[^m]$', '%H:%i', '%h:%i %p' )) ) ) - UNIX_TIMESTAMP() - {$this->getTimezoneShift()}) as time_left
					FROM {$wpdb->postmeta} postmeta
					INNER JOIN {$wpdb->posts} post ON (
														post.ID = postmeta.post_id
														AND
...
SQL;
			...
			$query .= <<<SQL
					WHERE
						...
						AND
							MONTH(STR_TO_DATE(meta_start_date.meta_value, '%m/%d/%Y') ) = {$month}
						AND
							YEAR (STR_TO_DATE(meta_start_date.meta_value, '%m/%d/%Y') ) = {$year}
					ORDER BY post_id
SQL;
	...
	/**
	 * Get start date meta key for current theme
	 * @return type
	 */
	private function getDateMetaKeyName()
	{
		return SHORTNAME . self::EVENT_START_DATE_META_KEY;
	}
	...
	/**
	 * Get end date meta key for current theme
	 * @return type
	 */
	...
	/**
	 * Get skipped dates meta key for current theme
	 * @return type
	 */
	private function getSkippedDatesMetaKeyName()
	{
		return SHORTNAME . self::EVENT_SKIPPED_DATES_META_KEY;
	}
	

	/**
	 * Get skipped dates array
	 * @return array
	 */
	public function getSkippedDates($event)
	{

		$skipped_dates = array();
		foreach ((array)explode(",", $event->skipped_dates) as $key => $skipped_date)
		{
			$skipped_dates[$key] = strtotime($skipped_date);
		}
