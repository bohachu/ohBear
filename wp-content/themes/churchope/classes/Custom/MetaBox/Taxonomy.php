...
//		wp_enqueue_script( 'jquery-ui-sortable' );
			...
			add_filter('init', array(&$this, 'init_upload_styles'));
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_file($field, $meta)
	{
		...
		if ($meta)
		{
			$img = '<div class="img_status"><img style="max-width:350px;" src="' . $meta . '"><a href="#" class="cmb_remove_file_button" rel="ch_term_menupattern">Remove Image</a></div>';
		}

		echo '<input class="cmb_upload_file" type="text" size="45" id="' . $field['id'] . '" name="' . $field['id'] . '" value="' . $meta . '">
					<input class="cmb_upload_button button" type="button" value="Upload File">
					<p class="cmb_metabox_description">You can upload custom image.</p>
					<div id="' . $field['id'] . '_status" class="cmb_upload_status">' . $img . '</div>';
		echo "</td>";
		echo "	<script>
					jQuery('.cmb_remove_file_button').live('click', function () {
									$('input#" . $field['id'] . "').val('');
									$(this).parent().remove();
									return false;
					});
					...
					jQuery('.cmb_upload_button').click(function(){
						var btnParent = jQuery(this).parent();
						var inputFile = jQuery(btnParent).find('.cmb_upload_file');
						var inputFileId = jQuery(inputFile).attr('id');
						...
						window.send_to_editor = function(html) {
							...
							if ($(html).html(html).find('img').length > 0) {
								...
								itemid = itemClassBits[itemClassBits.length - 2];
							...
							if(imgUrl && imgUrl.length) {
								...
								jQuery(btnParent).find('.cmb_upload_status').html('<div class = \"img_status\"><img style=\"max-width:350px;\" src=\"' + imgUrl + '\"/><a href=\"#\" class=\"cmb_remove_file_button\" rel=\"' + inputFileId + '\">Remove Image</a></div>');
								//jQuery('#" . $field['id'] . "').val(imgUrl);
								jQuery(inputFile).val(imgUrl);
							}
							tb_remove();
						};
						return false;
	...
	public function show_field_image($field, $meta)
	{
		...
		{
			$html .= "<span class='mupload_img_holder'><img src='" . $meta['src'] . "' style='height: auto;max-width: 300px;' /></span>";
			...
			$html .= "<input class='at-delete_image_button button' type='button' rel='" . $field['id'] . "' value='Delete Image' style='margin-left:10px' />";
		}
		else
		{
	...
	public function save_field_textarea($term_id, $field, $old, $new)
	{
