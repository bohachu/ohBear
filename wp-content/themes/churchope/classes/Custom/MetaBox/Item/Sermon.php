...
class Custom_MetaBox_Item_Sermon extends Custom_MetaBox_Item_Default
{
	
	function __construct($taxonomy)
	{
		parent::__construct($taxonomy);
		$this->setId('sermon_meta_box')
			->setTitle('Sermon Taxonomy Meta Box');
	...
	protected function addFields()
	{
		...
		$this->getMetaTaxInstance()->addSelect( SHORTNAME . '_tax_slider', array(''=>'Use global', 'revSlider'=>'revSlider', 'jCycle'=>'jCycle', 'Disable'=>'Disable'), array('name' => 'Slideshow type', 'std' => '', 'desc'=>'Select a slideshow type for current taxonomy'));
		$this->getMetaTaxInstance()->addSelect( SHORTNAME . Admin_Theme_Item_Slideshow::SLIDER, Admin_Theme_Element_Select_Slider::getSliders(), array('name' => 'Revolution Slider (if enabled)', 'std' => '', 'desc'=>'Select a revSlider for current category (if enabled)'));
		$this->getMetaTaxInstance()->addSelect( SHORTNAME . '_tax_slider_cat', $this->getCategoriesList(Custom_Posts_Type_Slideshow::TAXONOMY), array('name' => 'jCycle Slider Category (if enabled)', 'std' => ''));
		 ...
		 * @todo description
		 */
		$this->getMetaTaxInstance()->addParagraph( SHORTNAME . '_slider_effect_description', array('value'=>'jCycle Slideshow effects options'));
