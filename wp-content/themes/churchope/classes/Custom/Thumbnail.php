...
class Custom_Thumbnail
{
	...
	/**
		...
		return wp_get_attachment_image($thumb_id, $size_name);
	}
	...
	protected function isSizeChanged($size_name)
	{
		...
		if (is_array($current_attachment_meta) && isset($current_attachment_meta['sizes']) && key_exists($size_name, $current_attachment_meta['sizes']))
		{
			...
			{
				if ($theme_size[self::WIDTH] != $size_meta[self::WIDTH] || $theme_size[self::HEIGHT] != $size_meta[self::HEIGHT] || $theme_size[self::CROP] != $size_meta[self::CROP])
				{
					return true;
				}
			}
