...
class Custom_CSS_Style
{
	...
	private function getStyle()
	{
		...
		{
			$ch_gfont = Admin_Theme_Element_Select_Gfont::compatibility(isset($_SESSION[SHORTNAME . '_gfont']) ? stripslashes($_SESSION[SHORTNAME . '_gfont']) : get_option(SHORTNAME . '_gfont'));
			$this->options_value['ch_gfont'] = sprintf("'%s'", $ch_gfont[0]['family']);
		}

		$values = $this->options_value;
	...
	public function sessionJSPreview()
	{
		...
		if ($admin_menu && $admin_menu instanceof Admin_Theme_Menu)
		{
			...
			echo "\tvar google_font_list = '" . Admin_Theme_Element_Select_Gfont::FONT_STRING . "';\n";
			echo "</script>\n";
		}
	}
