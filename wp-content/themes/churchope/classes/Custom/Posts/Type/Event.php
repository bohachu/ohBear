...
class Custom_Posts_Type_Event extends Custom_Posts_Type_Default
{
	...
	{
		add_rewrite_rule(
				"{$this->getTaxSlug()}/([0-9]{4})/([0-9]{1,2})/([0-9]{1,2})/?$", 'index.php?pagename=customeventslist&event_year=$matches[1]&event_month=$matches[2]&event_day=$matches[3]', 'top');

		add_rewrite_rule(
				"{$this->getTaxSlug()}/([0-9]{4})/([0-9]{1,2})/([0-9]{1,2})/page/(\d+)/?$", 'index.php?pagename=customeventslist&event_year=$matches[1]&event_month=$matches[2]&event_day=$matches[3]&page=$matches[4]', 'top');
	}
...
//		$query_vars[] = 'event_pagination';
	...
	function th_post_type_columns($columns)
	{
		...
		$columns = array(
			...
			"thevents_event_skipped_dates" => __("Skipped dates:", 'churchope'),
		);
		...
		switch ($column)
		{
			...
			case "thevents_preview":
				...
				<?php if (has_post_thumbnail()) : ?>
					<a href="post.php?post=<?php echo $post->ID ?>&action=edit"><?php get_theme_post_thumbnail($post->ID, 'gallery_widget'); ?></a>
					<?php
				endif;
				break;
			...
			case 'thevents_event_start':
				echo get_post_meta($post->ID, SHORTNAME . Widget_Event::EVENT_START_DATE_META_KEY, true);
				break;
			...
			case 'thevents_event_end':
				...
				break;
			case 'thevents_event_skipped_dates':
				echo get_post_meta($post->ID, SHORTNAME . Widget_Event::EVENT_SKIPPED_DATES_META_KEY, true);
				break;
		}
	}
