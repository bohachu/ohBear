...
function th_contact_form_render_field($field)
{
	...
	switch ($field['type'])
	{
		...
		case 'checkbox':
			$r .= "\t<div class='form_line'><input type='checkbox' name='" . esc_attr($field_id) . "' value='" . __('Yes', 'churchope') . "' class='" . esc_attr($field['type']) . "' " . ( $field_value ? "checked='checked' " : "") . " /><label for='" . esc_attr($field_id) . "' class='" . esc_attr($field['type']) . "'>" . htmlspecialchars($field['label']) .
					//( $field['required'] ? '<span>' . __("(required)", 'churchope') . '</span>' : '' ) .
					"</label></div>\n";
			break;
...
// default values
	...
	$r .= $body;
	$r .= "\t<div class='th_contact-submit captcha captcha$captcha'>\n";
	$r .= "\t\t<input type='submit' value='" . htmlspecialchars($button_text) . "' class='churchope_button'/>";
	...
	if ($captcha)
	{
		if ($captcha_html = th_recaptcha_get_html())
		{
