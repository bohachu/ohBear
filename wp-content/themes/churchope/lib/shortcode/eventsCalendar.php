...
//	$tempDate			= mktime(0, 0, 0, $month);					// showing month
	 ...
	 */
	if ($from && strtotime($from) > mktime(0, 0, 0, $month, 1, $year) && strtotime($from) < mktime(23, 59, 59, $month, cal_days_in_month(CAL_GREGORIAN, $month, $year), $year))
	{
	...
	if ($calendar_events && is_array($calendar_events) && count($calendar_events))
	{
		...
		foreach ($calendar_events as $day_number => $events)
		{
			...
			$calendar_event = '';
			$calendar_events = ($events_count_for_day > 1 ) ? '<div class="other_events">' : '';
			if (
					...
					foreach ($events as $event)
					{
						...
						if ($details && isset($details['show_event']))
						{
							if ($details['show_event'] === true)
							{
								...
								if (!$calendar_event)
								{
									$calendar_event = '<div class="event"><a href="' . $details['url'] . '"><span class="time"><span>' . $details['time'] . '</span></span><span class="event_line"></span><span class="event_text">' . $details['title'] . '</span></a></div>';
								}
								else
								{
									$calendar_events .= '<div class="event"><a href="' . $details['url'] . '"><span class="event_text">' . $details['title'] . '</span><span class="time"><span>' . $details['time'] . '</span></span><span class="event_line"></span></a></div>';
								}
							}
						}
					}
					$calendar_events .= ($events_count_for_day > 1 ) ? '</div>' : '';
				}
...
//					}
					$calendar .= "<div class='day_wrapper'><span class='number'><span>$day_number</span>" . sprintf($href_to_events_for_the_day, '') . "</span>";
					$calendar .= $calendar_event . '</div>' . $calendar_events;
					if ($calendar_event && $events_count_for_day > 1)
					{
//						$calendar .= "<div class='multi_button'><span>" . __('More than one event in this day', 'churchope') . "</span>" . sprintf($href_to_events_for_the_day, __('view all events', 'churchope')) . "</div>";
						$calendar .= '<div class="more">' . sprintf($href_to_events_for_the_day, __('view all events', 'churchope')) . '</div>';
					}
					$calendar .= '</div>';
					$href_to_events_for_the_day = '';
				}
