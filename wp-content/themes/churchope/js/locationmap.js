function LocationMap(h, d) {
	...
	this.openMarker = function(e) {
		...
		if (a[e].url)
			d += "<a href='" + a[e].url + "' alt='" + a[e].title + "'>" + a[e].title + "</a>";
		else
			d += a[e].title;
		d += "</div>";
		...
		if (o)
			d += "as<a href='#' id='" + b + "_editmarker' alt='" + mapl10n.edit + "'>" + mapl10n.edit + "</a> | <a href='#' id='" + b + "_deletemarker' alt = '" + mapl10n.del + "'>" + mapl10n.del + "</a> | <a href='#' id='" + b + "_zoommarker' alt = '" + mapl10n.zoom + "'>" + mapl10n.zoom + "</a>";
		else if (n == "inline" || n == "google")
			d += "<a href='#location_map_0_directions' id='" + b + "_directionslink'>" + mapl10n.directions + "</a>";
		d += "</div>";
		d += "</div>";
		A(e);
		f.setContent(d);
	...
	this.geoCode = function(a, b, c) {
		...
		u.geocode({
			...
			for (var e = 0; e < d.length; e++)
				d[e].formatted_address == "" && d.splice(e, 1);
	...
	function k() {
		for (var h, c = "", f = 0; f < a.length; f++) {
			if (t)
				h = "<td class='location-map-marker'>[icon]</td><td><b>[title]</b>[bodytext]</td></tr>";
			else
			...
			if (a[f].title)
				if (a[f].url)
					g.title = "<a href='" + a[f].url + "' class='poi_list_title'>" + a[f].title + "</a><br/>";
				else
...
function LocationMapEditor(j, o) {
	...
	function h() {
		if (b === null || mQuery("#location_map_adjust_panel").is(":hidden"))
			return;
