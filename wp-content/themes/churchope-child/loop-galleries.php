...
<?php	while (have_posts()) : the_post();
		...
		foreach ( (array) wp_get_post_terms( get_the_ID(), Custom_Posts_Type_Gallery::TAXONOMY ) as $gallery_terms)
		{
			$cat_class[] = 'category-' . sanitize_html_class($gallery_terms->slug, $gallery_terms->term_id);
		}
		...
		elseif (is_archive()) :
			...
			switch ($gal_layout)
			{
				...
				case 'medium':
					 ...
					 $linebreak = ($wp_query->current_post % $num == 0  )? '':'';
					?>
					<article <?php post_class('gallery_listing  small grid_4 '.$linebreak. $cat_class) ?> id="post-<?php the_ID(); ?>">
				...
				case 'small':
					...
					<?php if (has_post_thumbnail())
										{ ?>
					<article <?php post_class('gallery_listing  small grid_4 '.$linebreak . $cat_class) ?> id="post-<?php the_ID(); ?>"><a href="<?php if ($preview_url)
							{
								echo $preview_url;
							} elseif (!$preview_url && $use_lightbox) {
				...
				<article <?php post_class('gallery_listing ' . $layout . $cat_class) ?> id="post-<?php the_ID(); ?>">
						<?php if (has_post_thumbnail())
						{ ?>		
						<a href="<?php if ($preview_url)
							{
