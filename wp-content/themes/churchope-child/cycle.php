...
{
	$slider_cat = get_option(SHORTNAME . "_blog_slider_cat");
	$slider_count = get_option(SHORTNAME . "_blog_slider_count");
}
...
if ($slider_query->have_posts()) :
	...
	elseif (is_home() && !get_post_meta(get_option("page_for_posts"), SHORTNAME . "_post_slider", true) && !get_option("page_on_front") && get_option(SHORTNAME . "_blog_slider") && get_option(SHORTNAME . "_blog_slider") !== "Disable" && get_option(SHORTNAME . "_blog_slider") !== "Use global")
	{
		$fx = get_option(SHORTNAME . "_blog_slider_effect");    
		$timeout = get_option(SHORTNAME . "_blog_slider_timeout");
		$speed = get_option(SHORTNAME . "_blog_slider_speed");
		$navigation = get_option(SHORTNAME . "_blog_slider_nav");
		$fixedheight = get_option(SHORTNAME . "_blog_slider_height");
		$padding = get_option(SHORTNAME . "_blog_slider_padding");
		$pause = get_option(SHORTNAME . "_blog_slider_pause");
		$autoscroll = get_option(SHORTNAME . "_blog_slider_autoplay");
	}
	else
	{
	...
	<div class="row" id="jcyclemain_navigation"><a href="#" id="slide_prev"></a><a href="#" id="slide_next"></a></div>
	<div id="jcyclemain">
	<?php
	while ($slider_query->have_posts()) : $slider_query->the_post();
		...
		$link_whole_target = (get_post_meta($post->ID, SHORTNAME . "_sliders_link_whole_target", true)) ? ' target="_blank" ' : '';
		...
		?>
			...
			if (has_post_thumbnail($post->ID) && ($cycle_position == 'left'))
			{
				if (!$usebg)
				{
					...
					if ($link)
					{
						echo '<a href="' . $link . '" ' . $link_whole_target . '>';
					}
					if ($slider_frame)
					{
			...
			{
				?>
							<div class="cycle_content cycle_col <?php echo ($cycle_position == 'empty' ) ? 'grid_12' : 'grid_6';
				echo ' ' . $content_align; ?>">
							<?php if (!$slider_title)
							{ ?>
						...
						if ($link_whole && $link)
						{
							echo '<a href="' . $link . '" class="slide_link_whole"' . $link_whole_target . '></a>';
						}
						?>
				</div>
			</div>
