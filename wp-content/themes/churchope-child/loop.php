...
$(document).ready(function() {
  $('#ohbear-activity-single-gallery a').click(function(evt) {
  evt.preventDefault();
  var imgPath = $(this).attr('href');
  var oldImage = $('#ohbear-activity-single-photo img');
  var newImage = $('<img src="' + imgPath +'">');
  newImage.hide();
    
  $('#ohbear-activity-single-photo').prepend(newImage);
  newImage.fadeIn(1);
  oldImage.fadeOut(1,function(){
   $(this).remove();
  }); // end fadeout 
 }); // end click
 $('#ohbear-activity-single-gallery a:first').click();    
} );// end ready
</script>

<script>
$(document).ready(function() {
   $('#ohbear-activity-single-gallery img').click(function(evt) {
   evt.preventDefault();
    $('#ohbear-activity-single-gallery img').css('opacity', '0.3');
    $(this).css('opacity', '1');
    }); // end click  
  $('#ohbear-activity-single-gallery img:first').click();
} );// end ready
</script>     
           
            
							<?php
							if (get_the_tags() && !get_option(SHORTNAME."_disable_tags"))
							{ ?>
								<p class="post_tags">
								<?php
								the_tags();
							...
							<?php } ?>
								<?php if ( !get_option(SHORTNAME."_disable_cats")) { ?>
						<p class="post_categories"><?php _e('Categories:', 'churchope'); ?> <?php the_category(','); ?></p>
								<?php } ?>
					</div>
			...
			if (comments_open()) : comments_popup_link(__('Comments (0)', 'churchope'), __('Comment (1)', 'churchope'), __('Comments (%)', 'churchope'), 'commentslink');
				...
				?><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to', 'churchope'); ?> <?php the_title_attribute(); ?>"  class=""><?php _e('Read more', 'churchope'); ?></a></p>
						...
						<div class="entry-content  <?php if (!get_option(SHORTNAME . "_excerpt")) {echo " entry-excerpt";} ?>">
							...
							if (get_option(SHORTNAME . "_excerpt"))
							{
								the_content(__('...','churchope'), false);
							}
							else
							{
								the_excerpt();
